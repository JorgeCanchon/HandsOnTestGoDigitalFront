...
const searchName = async () => {
    let name = document.getElementById('nameMovie').value.trim();
    if (name == '' || name == undefined) {
        alert('El campo de búsqueda no debe estar vacio');
        return false;
    }  
    let url = `${URL}search/movie?api_key=${APIKEY}&query=${name}&page=1`;
    let result = await fetch(url);
    let data = await result.json();
...
const appenedData = data => {
    ...
    data.forEach(x => {
        ...
        let width = x.poster_path != null ? '50': '100';
        $('#carouselIndicators').append(templateCarouselIndicators(i++));
        $('#carouselInner').append(templateCarouselInner(srcImage, x.title, x.overview, width, active));
    });
}
...
const templateCarouselInner = (srcImage, title, overview, width, active = '') => (`<div class="carousel-item ${active}">
        <img class="d-block w-${width} rounded img-responsive" data-src="holder.js/800x400?auto=yes&amp;bg=777&amp;fg=555&amp;text=${title}" alt="${title} [800x400]" src="${srcImage}" data-holder-rendered="true">
        <div class="carousel-caption d-none d-md-block">
            <h5 style='color: #a1b3c0;'>${title}</h5>
            <p style='color: #a1b3c0;'>${overview}</p>
        </div>
    </div>`);
